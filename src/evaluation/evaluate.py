...
def evaluate_model(
    ...
    output_dir = output_dir or (PROJECT_ROOT / "runs" / "evaluation" / weights_path.parent.parent.name)
    ...
    if metrics_result is not None:
        ...
        metrics = {
            f"test/mAP50":       rd.get("metrics/mAP50(B)",    0.0),
            f"test/mAP50_95":    rd.get("metrics/mAP50-95(B)", 0.0),
            f"test/precision":   rd.get("metrics/precision(B)", 0.0),
            f"test/recall":      rd.get("metrics/recall(B)",    0.0),
            f"test/F1":          compute_f1(
...
def evaluate_all(
    ...
    for weights_path in weight_files:
        ...
        arch     = "rtdetr" if "rtdetr" in run_name else "yolov11" if "yolov11" in run_name else "yolov8"
...
def print_comparison_table(results: List[Dict]) -> None:
    ...
    cols = [
        "model", "arch",
        "test/mAP50", "test/mAP50_95", "test/precision", "test/recall", "test/F1",
        "inference_fps", "model_size_mb", "n_parameters_M",
    ]
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--evaluate-all", action="store_true",
                   help="Evaluate all best.pt files in runs-dir")
    return p.parse_args()
